...
DROP TABLE IF EXISTS `university_khl`.`student`;
CREATE TABLE `university_khl`.`student` (
  ...
  -- world DB가 선택된 상태에서 create table university_khl.student();를 하면 
  -- 선택된 DB와 상관없이 university_khl DB안에 student 테이블을 생성

-- 4. professor 테이블 생성  
DROP TABLE IF EXISTS `university_khl`.`professor`;
CREATE TABLE `university_khl`.`professor` (
...
-- 5. subject 테이블 생성
DROP TABLE IF EXISTS `university_khl`.`subject`;
CREATE TABLE `university_khl`.`subject` (
  ...
  PRIMARY KEY (`su_num`));
-- 5. course 테이블 생성
DROP TABLE IF EXISTS `university_khl`.`course`;
CREATE TABLE `university_khl`.`course` (
  ...
  CONSTRAINT `co_st_num` FOREIGN KEY (`co_st_num`) 
    REFERENCES `university_khl`.`student` (`st_num`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `co_su_num` FOREIGN KEY (`co_su_num`)
    REFERENCES `university_khl`.`subject` (`su_num`) ON DELETE CASCADE ON UPDATE CASCADE);
-- 6. lecture 테이블 생성
DROP TABLE IF EXISTS `university_khl`.`lecture`;
CREATE TABLE `university_khl`.`lecture` (
  ...
  CONSTRAINT `le_pr_num` FOREIGN KEY (`le_pr_num`)
    REFERENCES `university_khl`.`professor` (`pr_num`) ON DELETE CASCADE ON UPDATE CASCADE,
