...
//		System.out.println(GreanClass2.get("V").toString());
		...
		while(flag)
		{
			...
			switch(num)
			{
				...
				case 3: correctStd(GreanClass, scan); break;
				case 4: deleteStd(GreanClass, scan); break;
				case 5: flag = false; System.out.println("종료합니다"); break;
	...
	public static void correctStd(List<ExbStudents> list, Scanner scan)
	{
		...
		{	
			...
			String info[] = new String[5];
			info = getInfo.split(",");
			list.get(list.indexOf(std)).setName(info[0]);
			list.get(list.indexOf(std)).setKorean(Integer.parseInt(info[1]));
			list.get(list.indexOf(std)).setMath(Integer.parseInt(info[2]));
			list.get(list.indexOf(std)).setEnglish(Integer.parseInt(info[3]));
