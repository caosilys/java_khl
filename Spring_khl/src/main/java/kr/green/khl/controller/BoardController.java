...
public class BoardController {
	...
	public ModelAndView boardList(ModelAndView mv, PageMaker pm, Integer page, String search, String type) {		
		
		// 페이지 메이커를 세팅 (서비스에서 전체 글 수를 가져옴)
		pm = boardService.setPageMaker(pm, page, search, type);
	...
	public ModelAndView boardRegisterGet(ModelAndView mv, Integer bd_ori_num, String type, HttpServletRequest request) {
		
		MemberVO user = (MemberVO)request.getSession().getAttribute("user");
		if(checkAdmin(user, type)) {
	...
	public ModelAndView boardRegisterPost(ModelAndView mv, BoardVO board, HttpServletRequest request, List<MultipartFile> file) throws Exception {
		...
		if(checkAdmin(user, board.getBd_type())) {
...
//		    String uploadPath = "C:\\Users\\green\\Desktop\\upload";
		...
		private boolean checkAdmin(MemberVO user, String type) {
			// 공지가 아닌 글은 누구나 쓸 수 있음
			if(type == null) return true;
			if(!type.equals("공지")) return true;
			
			// 관리자나 슈퍼관리자는 "공지" 인 글도 쓸수있음
			if(user.getMe_authority().equals("관리자")) return true;
			if(user.getMe_authority().equals("슈퍼관리자")) return true;		
